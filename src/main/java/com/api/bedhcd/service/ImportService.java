                                ...
                                User user = userRepository.findByCccd(record.getCccd())
                                                .orElseGet(() -> {
                                                        ...
                                                        User newUser = User.builder()
                                                                        .id(RandomUtil.generate6DigitId(
                ...
                for (ProxyImportRecord record : records) {
                        try {
                                ...
                                User delegatorUser = userRepository.findByCccd(record.getDelegatorCccd())
                                                .orElseThrow(
                                                                () -> new BadRequestException("Delegator not found: "
                                                                                + record.getDelegatorCccd()));

                                User proxyUser = userRepository.findByCccd(record.getProxyCccd())
                                                .orElseGet(() -> {
                                                        log.info("Creating new proxy user for CCCD={}",
                                                                        record.getProxyCccd());
                                                        log.debug("Proxy user details - FullName: {}, Email: {}, DateOfIssue: {}",
                                                                        record.getFullName(), record.getEmail(),
                                                                        record.getDateOfIssue());

                                                        Set<Role> roles = new HashSet<>();
                                                        roles.add(Role.REPRESENTATIVE);

                                                        String generatedId = RandomUtil
                                                                        .generate6DigitId(userRepository::existsById);
                                                        ...
                                                        User newUser = User.builder()
                                                                        ...
                                                                        .fullName(record.getFullName())
                                                                        .email(record.getEmail() != null
                                                                                        && !record.getEmail().isEmpty()
                                                                                                        ? record.getEmail()
                                                                                                        : record.getProxyCccd()
                                                                                                                        + "@bedhcd.com")
                                                                        .phoneNumber(record.getProxyCccd())
                                                                        .address("N/A")
                                ...
                                if (record.getSharesDelegated() > delegator.getSharesOwned()) {
                                        throw new BadRequestException("Not enough shares for delegation by "
                                                        + record.getDelegatorCccd());
                                }
                                ...
                                log.info("Success: Delegated {} shares from {} to {}",
                                                record.getSharesDelegated(), record.getDelegatorCccd(),
                                                record.getProxyCccd());

                        } catch (Exception e) {
                                log.error("Error processing proxy record for delegator {}: {}",
                                                record.getDelegatorCccd(),
                                                e.getMessage());
                                throw e;
                        }
